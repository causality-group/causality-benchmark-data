...
def calculate_performance_df(    
    ...
    # PnL Standard Deviation
    performance_df['Annual Standard Deviation'] = pnl_df.std() * np.sqrt(num_periods_per_year)

    # Sharpe Ratio
    performance_df['Annual Sharpe Ratio'] = pnl_df.mean() / pnl_df.std() * np.sqrt(num_periods_per_year)

    # Sortino Ratio
    downside_returns = pnl_df[pnl_df < 0]
    downside_std = downside_returns.std()
    performance_df['Sortino Ratio'] = pnl_df.mean() / downside_std  * np.sqrt(num_periods_per_year)
