def signalbacktest_df(
    signal_df, upcoming_ret_df, enter_at_stdev=2.0, do_return_signal=False
):
    ...
    is_long_df = (
        signal_df.add(
            -(signal_df.mean(axis=1) + enter_at_stdev * signal_df.std(axis=1)), axis=0
        )
        > 0.0
    )
    is_short_df = (
